...
module.exports = {
   ...
   * @return {object} Request promise
   */
  weather: function(commandArguments) {
  	//console.log('Function Args %j', commandArguments);
  	var location = '';
  	for (i = 0; i < commandArguments.length; i++) {
    	location += commandArguments[i] + ' ';
	}
    ...
    var dataApi = config.weatherApiUrl + '&q=' + location;
    return rp({
      uri: dataApi,
      json: true
    }).then(function(body) {
      ...
      if (body.list &&
          body.list.length > 0) {
        body.list.forEach(function(nowcast) {
          //console.log('Loop data %j', nowcast.weather[0].main);
          switch(nowcast.weather[0].main) {
    		case "Clear":
        		var icon = ' :sunny:'
        		break;
    		case "Clouds":
          ...
          fields.push(
            {
              title: helper.formatDate(nowcast.dt),
              value: icon + helper.formatNumber(nowcast.temp.day) + 'ºC' + ' - ' + helper.formatNumber(nowcast.temp.night) + 'ºC',
              short: true
            }
          );
        });
      }
      var cName = body.city.name + ', ' + body.city.country;

      // Attachments
      var attachments = [{
        pretext: ':sunny: :cloud: :rain_cloud: *' + cName + ' Weather Conditions*',
        title: body.cnt + ' Days Forecast',
